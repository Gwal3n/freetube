...
- **Player gestures live in `contentOverlayView`.** `PlayerGestureCoordinator` adds double-tap left/right to seek −/+10 seconds and a 0.35-second hold for temporary 2× playback, restoring the prior rate on release. Recognizers must remain simultaneous and non-cancelling so native controls, scrubbing, AirPlay, and PiP continue to work.
