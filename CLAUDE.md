...
- **Mini-player title marquee uses `MiniPlayerMarqueeText`.** LNPopupController's native marquee label can still receive an undersized vertical line box on iOS 26, clipping both the title and subtitle even on 4.5.9. The popup item therefore supplies vertically fixed SwiftUI title/subtitle views with a seamless duplicated-text marquee; do not switch these back to the string-based `.popupTitle` overload. The popup packages remain pinned to LNPopupUI 4.0.1 / LNPopupController 4.5.9. Its small explicit X is a leading popup button before the thumbnail and calls `PlayerStateManager.dismiss()` so playback and popup state are torn down together.
- **`AVPlayerViewController`** wrapped in `UIViewControllerRepresentable` (`PlayerSurface.swift`) remains the video-rendering and automatic-PiP engine, with `showsPlaybackControls = false` and video-frame analysis disabled. `CustomPlayerControls` owns the visible YouTube-style play/pause, previous/next buttons, speed menu, fullscreen orientation toggle, auto-hide chrome, and `SponsorBlockTimeline`. Controls use unbacked white glyphs with shadows rather than dark button circles. Fullscreen requests landscape-left (notch left) and toggles back to portrait. More-actions and fullscreen are the only top player controls; native bordered Share and Download controls sit below the metadata. Share consolidates the system share sheet, browser, clipboard, timestamp, and downloaded-file actions. Download calls the existing `DownloadManager.ensureDownloaded` yt-dlp path and reflects active/completed state. There is no separate transport bar below the video and no bottom Close pill. The controls remain mounted and fade by opacity so their layout does not churn during auto-hide. The video remains full-width in compact-height landscape; only the timeline is lifted by the portion of the 16:9 surface extending below the viewport. The timeline seeks only on drag end, places time labels above the track, and renders enabled SponsorBlock segments at exactly the track's thickness. `PlayerStateManager.seek` pins its optimistic target until AVPlayer confirms the seek (plus a short stale-tick drain), preventing the scrubber from flashing back to the old periodic-observer position. Automatic PiP-on-background is enabled; there is intentionally no AirPlay or manual PiP button.
...
- **Search is search-only.** Its idle state contains local recent searches or the clean empty state;
  ...
  `.searchable` (including the system Liquid Glass treatment) with an always-visible navigation
  drawer placement; Mac uses an inline field because native search collapses awkwardly there. A
  simultaneous observer installed on the window's native `UITabBar` detects re-tapping the selected
  Search tab and drives `.searchable(isPresented:)`. Selecting a suggestion, history entry, or video
