...
Download actions first resolve the same native source used by playback, then give that resolved media URL directly to embedded yt-dlp for concurrent HLS fragment transfer. yt-dlp does not extract the YouTube watch page on this primary path; the serialized in-process FFmpeg runner only muxes the downloaded tracks. Page extraction is a compatibility fallback and must not enable `formats=missing_pot`: those formats are intentionally filtered because they lack a required Proof-of-Origin token and produce HTTP 403 or throttled transfers.
