...
- **Background audio:** `AudioSessionConfigurator` runs at app launch with `(.playback, .moviePlayback)`. The optional “Allow audio from other apps” preference adds `.mixWithOthers` and reapplies the session immediately; it defaults off so FreeTube retains normal primary-media behavior. While mixing, the other app may own Now Playing. `AppEnvironment` observes `silenceSecondaryAudioHintNotification`; when the competing primary audio stops, it reactivates the still-mixable session and republishes FreeTube's current metadata so FreeTube can reclaim Now Playing without interrupting simultaneous playback.
