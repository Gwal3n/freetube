...
- **Mini and expanded presentation is owned by `SwiftUIPlayerContainer`.** The same `AVQueuePlayer` instance remains mounted in `FullScreenPlayer`; the mini-player is lightweight SwiftUI chrome and artwork, never a second player. Interactive vertical drags track the finger and can be reversed before release. The container respects the details panel's scroll-origin flags and yields entirely while Chapters or portrait fullscreen owns vertical gestures.
- **Direct video selections open the expanded player immediately.** `PlayerStateManager.load` defaults `expandPlayer` to `true`; automatic next/previous transitions pass `false` so they preserve the presentation state the user chose. Downward mini-player dismissal must call `PlayerStateManager.dismiss()` so playback, resolution, Now Playing, and presentation state are torn down together. Upward drags and tapping the artwork/title expand it.
- **Mini-player title/subtitle are static SwiftUI labels.** Keep them single-line and vertically centred; do not add timer-driven marquee state because earlier implementations clipped, accelerated, or disappeared as playback metadata changed. Its explicit X remains before the thumbnail and calls `PlayerStateManager.dismiss()`.
- **On iOS 26 the mini-player uses `tabViewBottomAccessory`.** Do not estimate the native tab bar's height with fixed bottom padding. The system accessory is what keeps Liquid Glass positioned above the normal tab bar and inline when the tab bar collapses. iOS 17–25 retain a material-overlay fallback. Mini-player Liquid Glass versus OLED black is selected independently from the expanded player's OLED background.
...
- **Expanded player details are resilient and optionally prefetched.** Tapping the video title toggles the description
  ...
  Suspend the SwiftUI container's global content drag only while that browser is presented so the gestures do
