...
- **Long comments stay compact by default.** `CommentRow` limits long bodies to four lines and exposes an inline Read more / Show less control. The explicit comment continuation remains a compact neutral Load more button; it must never become an eager scroll sentinel.
