...
- **Mini-player title marquee stays disabled.** Stable truncation is preferable to moving text in the compact bar. The popup packages are pinned to LNPopupUI 4.0.1 / LNPopupController 4.5.9 because the older controller 4.4.1 predates several iOS 26 bar-layout fixes and the updated system marquee-label implementation; do not downgrade it when resolving packages.
