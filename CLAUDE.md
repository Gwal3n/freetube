...
- **Search is search-only.** Its idle state contains local recent searches or the clean empty state;
  ...
  native search collapses awkwardly there. Do not bind `.searchable(isPresented:)` or install a tab
  bar gesture observer: both interfere with normal field taps on iOS 26. Selecting a suggestion,
  history entry, or video explicitly resigns keyboard focus without dismissing the search UI.
