...
Download actions first resolve the same native source used by playback and materialize it with the serialized in-process FFmpeg runner. This normally means downloading the valid HLS manifest already proven by AVPlayer. Embedded yt-dlp is a compatibility fallback and must not enable `formats=missing_pot`: those formats are intentionally filtered because they lack a required Proof-of-Origin token and produce HTTP 403 or throttled transfers.
