...
Download actions first resolve the same native source used by playback, then pass it to `NativeHLSDownloadService`. That service parses the HLS master/media playlists, chooses the quality-bounded video and default audio renditions, downloads segments concurrently with `URLSession`, assembles tracks on disk, and uses the serialized in-process FFmpeg runner only for the final mux. Embedded yt-dlp does not participate in this primary transfer path. Page extraction remains a compatibility fallback and must not enable `formats=missing_pot`: those formats are intentionally filtered because they lack a required Proof-of-Origin token and produce HTTP 403 or throttled transfers.
