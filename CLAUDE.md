...
- **Background audio:** `AudioSessionConfigurator` runs at app launch with `(.playback, .moviePlayback)`. The optional “Allow audio from other apps” preference adds `.mixWithOthers` and reapplies the session immediately; it defaults off so FreeTube retains normal primary-media behavior.
