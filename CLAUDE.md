...
- **Mini-player title marquee stays disabled.** LNPopupUI's MarqueeLabel animation clips long video titles vertically and can eventually leave the title blank while the uploader subtitle remains fine. Keep `.popupBarMarqueeScrollEnabled(false)` on the popup container; stable truncation is preferable to disappearing text.
