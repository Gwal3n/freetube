...
- **Mini-player title marquee uses LNPopupUI's default scrolling behavior.** The popup packages are pinned to LNPopupUI 4.0.1 / LNPopupController 4.5.9 because the older controller 4.4.1 vertically clipped and eventually hid the title/subtitle on iOS 26; do not downgrade it when resolving packages. Its small explicit X is a leading popup button before the thumbnail and calls `PlayerStateManager.dismiss()` so playback and popup state are torn down together.
