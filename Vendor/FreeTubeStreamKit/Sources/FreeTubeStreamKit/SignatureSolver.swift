...
class SignatureSolver {
    ...
    private func solve(with input: Input) throws -> Response {
        ...
        guard let json = try String(data: encoder.encode(input), encoding: .utf8) else {
        ...
        do {
            ...
            // The solver output contains short-lived signature material. Never write it to logs.
            os_log("JSC output could not be decoded", log: Self.log, type: .error)
